...
public class old_pistol : MonoBehaviourPun
{
    ...
    void Update()
    {
        ...
        // walk status, if the weapon is filled 
        ...
        // walk status, with a empty weapon
        if (walking && !in_reload && magazine_current < 1)
        {
            if (!button_aim)
            {
                ...
                //setting the animation speed lower, if we walk in status "duck"
                if (duck_walk)
                {
                    ani.SetFloat("weapon_speed", 0.5f);
                }
        ...
        // running + unempty magazine
        if (running && !in_reload && magazine_current > 0)
        {
            // running full magazine
            ani.SetInteger("old_pistol", 12);



        }
        // running + empty magazine
    ...
    public IEnumerator reload_start()
    {
        ...
        {
            reload_time = 1.37f;
            ani.SetInteger("old_pistol", 10); // unempty reload 
        ...
        {
            reload_time = 1.16f;
            ani.SetInteger("old_pistol", 11); // empty reload
