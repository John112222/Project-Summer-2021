...
namespace LevelGenerator.Scripts
{
    public class LevelGenerator : MonoBehaviour
    {
        ...
        [SerializeField] bool generateLevelOnStart = false;

        protected void Start()
        {
            if (generateLevelOnStart) GenerateLevel();
        }
        ...
        public void DestroyLevel()
        {
            ...
            DeadEndColliders.Clear();
            LevelSize = 0;
            foreach (TagRule rule in SpecialRules)
            {
                rule.RemoveRuleSection();
            }
        }
            ...
            if (RulesContainTargetTags(tags) && HalfLevelBuilt)
            {
                foreach (var rule in SpecialRules.Where(r => r.NotSatisfied))
                {
                    if (tags.Contains(rule.Tag))
                    {
                        ...
                        || (x.GetComponent<SectionVariaet>()?.variants.Any(y => y.addTags.Contains(rule.Tag)) ?? false)).PickOne();
                    }
                }
            }
